...
export default function FlashSalesPage() {
  ...
  const addFlashSaleMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || 'Failed to add flash sale')
    },
  })
  ...
  const removeFlashSaleMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || 'Failed to remove flash sale')
    },
  })
  ...
  const handleRemoveFlashSale = () => {
    ...
    if (confirm(`Remove ${selectedProducts.length} product(s) from flash sale?`)) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Ready to add to sale</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Selected Products</CardTitle>
      ...
      {selectedProducts.length > 0 && (
        <Card className="border-oud-gold">
          <CardHeader>
            <CardTitle className="text-lg">Bulk Actions ({selectedProducts.length} selected)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <Label htmlFor="endDate">Sale End Date (Optional)</Label>
                <div className="flex items-center gap-2 mt-1.5">
                  <Input
                    id="endDate"
                    type="datetime-local"
              ...
              <div className="flex items-end gap-2">
                <Button
                  ...
                  variant="default"
                >
                  {addFlashSaleMutation.isPending ? 'Adding...' : 'Add to Flash Sale'}
                </Button>
                <Button
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Products currently on flash sale</CardDescription>
            </div>
        ...
        <CardContent>
          {loadingFlashSale ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mt-1">Select products below to create a flash sale</p>
            </div>
          ) : (
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Add products to flash sale</CardDescription>
            </div>
        ...
        <CardContent>
          {loadingAllProducts ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mt-1">All your products are already on flash sale</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {expiredFlashSales.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Expired Flash Sales</CardTitle>
            <CardDescription>Past flash sales that have ended</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
